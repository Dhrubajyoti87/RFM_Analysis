...
hist(customers$amount, breaks = 50)
...
z = kmeans(new_data, 6, nstart = 25)

print(z)

cluster_data = aggregate(customers[, 2:4], by=list(cluster=z$cluster), mean)
cluster_data
...
cluster_data$percentage = table((z$cluster)/18417 * 100)
cluster_data
...
fviz_cluster(z, data = new_data,
             palette = c("#E7298A", "#00AFBB", "#E7B800","#00FF00","#FF0000","#0000FF"), 
             geom = "point",
             ellipse.type = "convex", 
             ggtheme = theme_bw()
)
...
AB = table(z$cluster)/18417 * 100

df_cl <- data.frame(matrix(unlist(AB), nrow=length(AB), byrow=TRUE))
df_cl


cluster_data$percent = df_cl
cluster_data
